...
//Functionalities for connecting our agent to our call 
...
export async function POST(req: NextRequest) {
    ...
    if (eventType === "call.session_started") {
        ...
        //connect the client with openai
        const realtimeClient = await streamVideo.video.connectOpenAi(
            { call, openAiApiKey: process.env.OPENAI_API_KEY!, agentUserId: existingAgent.id }
        )
        ...
        const event = payload as CallSessionStartedEvent
        const meetid = event.call_cid.split(":")[1] // call_cid is formatted as "type:id"

        if (!meetid) {
            return NextResponse.json({ error: "Missing meetingId" }, { status: 400 })
        }

        const call = streamVideo.video.call("default", meetid);
        await call.end();
        ...
        const [updatedMeeting] = await db
            .update(meetings).set({ transcriptURL: event.call_transcription.url })
