...
export const meetingProcessing = inngest.createFunction(
    ...
    async ({ event, step }) => {
        //const response = await step.fetch(event.data.transcriptUrl) // in prodcution this will break 
        ...
        const transcript = await step.run("parse-transcript", async () => {
            ...
            // the problem is currently we don't know the type , thanks to jsonl we can add the type 
        });
        ...
        const { output } = await summarizer.run("Summairze the following transcript: " + JSON.stringify(transcriptWithSpeakers))
